...
//        }
    ...
    private HashMap<String, Set<Sentence>> groupSentencesByVar() {
        ...
        allVars.addAll(primaries);
        allVars.addAll(secondaries);
    ...
    private Set<Sentence> fuseObjectWithSubject(Set<Sentence> sentences, Set<String> fused) {
        ...
        if (objsent == null || subjsent == null || object == null || subject == null || object.equals(subject))
            return sentences;
...
//                     if (m.find()) equalities.put(var,equalities.get(var).replace(m.group(1),""));
            ...
            bodyParts.removeAll(deleteThese);
            deleteThese = new HashSet<String>();
            bodyParts.addAll(addThese);
            addThese = new HashSet<String>();
            ...
            for (String var : vars) {
                ...
                for (NPPhraseSpec sel : selects) {
                    ...
                    if (head.contains(var)) {
                        ...
                        if (realiser.realise(sel).toString().isEmpty())
                            sel.setFeature("head", "entities"); // happens in the case of ASK queries, where selects have no premodifier
                        inSelects = true;
                        break;
                    }
                }
...
//                 m_p_var_s = Pattern.compile(p_var_s).matcher(b);
            ...
            if (!realiser.realise(additionaloutput).toString().matches(".*\\?" + s + "(\\'|\\s|\\z).*")) {
                ...
                if (count > 0) {
                    ...
                    for (String b : bodyParts) {
                        ...
                        String new_b = "";
                        if (b.matches(".*\\?" + s + "(\\'|\\s|\\z).*")) {
                            if (initial && !firstOccurence) {
                                firstOccurence = true;
                                initial = false;
                            } else if (!initial && firstOccurence) {
    ...
    private void aggregateTypeAndLabelInformation(String var) {
        ...
        if (type != null || label != null || name != null) {
            ...
            sentences.remove(type);
            sentences.remove(label);
            sentences.add(new Sentence(newsentence, false, id));
            id++;
        }
    }
                ...
                for (Sentence s : f.sentences) {
                    ...
                    if (fstring.startsWith(var + " does not exist")) {
                        ...
                        if (inSelects(subj)) sentence.sps.setFeature("negated", true);
                            ...
                            sentence.sps.setSubject("no " + removeDots(realiser.realise(sentence.sps.getSubject()).toString()));
...
//                    boolean part2postmod = false;
                        if (m.group(2) != null) {
                            part2 = m.group(2);
                            if (m.group(2).endsWith(" and " + m.group(1)))
                                part2 = part2.substring(0, part2.lastIndexOf(" and "));
...
//                        else 
                        ...
                        if (!restrealization.isEmpty()) {
                            if (restrealization.endsWith(" and"))
                                restrealization = restrealization.substring(0, restrealization.length() - 4);
                        ...
                        for (NPPhraseSpec sel : selects) {
                            if (sel.getFeatureAsString("head").equals(m.group(1))) {
                                ...
                                if (rest != null) keepuri = true;
                                    ...
                                    if (realiser.realiseSentence(additionaloutput).matches("(\\A|(.*\\s))\\" + m.group(1) + "(\\s|\\z|').*"))
                                        keepuri = true;
                                }
...
//                            }
                                ...
                                if (sel.hasFeature("premodifiers")) {
                                    ...
                                    if (!premods.isEmpty() && premods.get(0).hasFeature("number")) {
                                        if (premods.get(0).getFeatureAsString("number").equals("SINGULAR"))
                                            pron = "its";
                                    }
                                }
